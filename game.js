...
function getPaddleHitPoints() {
  return BASE_HIT_POINTS * (doublePointsRound ? 2 : 1);
}

function handleMiss(side) {
  if (side === 'left') {
    if (leftLives <= 0) return;
    leftLives -= 1;
    if (leftLives <= 0) {
      gameState = 'won';
      winnerTitle.textContent = botActive ? 'BOT WINS!' : 'RIGHT WINS!';
      winnerScore.textContent = `${leftScore} — ${rightScore}`;
      showMenu(winnerMenu);
    }
  } else {
    if (rightLives <= 0) return;
    rightLives -= 1;
    if (rightLives <= 0) {
      gameState = 'won';
      winnerTitle.textContent = botActive ? 'YOU WIN!' : 'LEFT WINS!';
      winnerScore.textContent = `${leftScore} — ${rightScore}`;
      showMenu(winnerMenu);
    }
  }
...
let leftLives = MAX_LIVES, rightLives = MAX_LIVES;
...
function buyPowerup(side, powerupKey) {
  ...
  if (powerupKey === 'extraLife') {
    if (side === 'left') {
      leftCoins -= pu.cost;
      leftLives = Math.min(MAX_LIVES, leftLives + 1);
    } else {
      rightCoins -= pu.cost;
      rightLives = Math.min(MAX_LIVES, rightLives + 1);
    }
...
function startGame(withBot) {
  ...
  leftLives  = MAX_LIVES; rightLives = MAX_LIVES;
...
function drawHUD() {
  ...
  ctx.textAlign = 'left';
  ctx.fillStyle = '#555';
  ctx.fillText(`¢ ${leftCoins}  ♥ ${leftLives}`, 22, 78);
  ...
  ctx.textAlign = 'right';
  ctx.fillStyle = '#555';
  ctx.fillText(`¢ ${rightCoins}  ♥ ${rightLives}`, W - 22, 78);
